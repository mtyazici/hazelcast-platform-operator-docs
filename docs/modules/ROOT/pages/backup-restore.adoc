...
<1> The `HotBackup` resource name the that will be used for restore. The backup folder name will be taken from the `HotBackup` resource.
...
<1> The `HotBackup` resource name the that will be used for restore. The bucket URI and secret will be taken from the `HotBackup` resource.
<2> The agent which is responsible for restoring data from the external storage.
    The agent configuration is optional. If you give `restore` under the `persistence` and do not pass the agent configuration, the operator
    will use the latest agent version compatible with the operator version.
--
